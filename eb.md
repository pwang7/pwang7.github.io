...
EB定义了三个阶段，分别为阶段0、阶段1、阶段2，每时每刻EB只能处于一个阶段。同时EB给每个待回收对象都标识其所处阶段。
...
```
上面的代码片段里可以看到对于入栈`push()`和出栈`pop()`这两个函数，都用`EB::enter()`和`EB::leave()`把对共享数据——无锁化堆栈的修改操作标识出来。
入栈操作不产生待回收对象，出栈操作里把出栈的栈顶节点的指针传给`EB::enter()`，由EB后续回收该节点的内存。
虽然`EB::enter()`和`EB::leave()`有点像是访问由锁保护的临界区以修改共享数据：
```
...
接下来每个线程把要访问共享数据进行修改的操作，都用`EB::enter()`和`EB::leave()`标识出来，并且把待回收的对象指针通过`EB::leave()`传给EB来后续回收。
...
在第3步，`EB::enter()`先检查阶段跨越的条件，满足条件时全局阶段进入下一个阶段，然后回收全局阶段的两个阶段前的对象。
由此，EB回收内存的确定性指的就是`EB::enter()`会负责回收内存，而且`EB::enter()`是批量回收，即批量回收全局阶段的两个阶段前的对象。
...
首先，在一个线程里，其线程函数的操作是串行执行的。
由于一个线程内部操作的串行执行，当一个线程处于某个阶段，那么这个线程不会还保留着前一个阶段的待回收对象的指针，
不然这个对象就不是待回收的，该对象应该是线程仍然在访问的对象。
线程还在访问的对象是不应该交给EB来回收的，只有线程不再访问的对象才应该交给EB来回收。
因此，对每个线程来说，其当前本地阶段的前一个阶段的待回收对象是有可能被安全回收的，至少当前线程不会再访问前一个阶段的待回收对象。

其次，EB的阶段跨越条件保证了，每个线程的本地阶段要么跟全局阶段一致，要么处于全局阶段的前一阶段，不可能出现某个线程的本地阶段处于全局阶段的前两个阶段。
回顾`EB::enter()`里第三步检查阶段跨越的条件：
- 所有线程的本地阶段都一致。

阶段跨越的条件，要求全局阶段跨越至下个阶段前，所有线程的本地阶段都一致。
这就保证了线程的本地阶段最多落后全局阶段一个阶段，不会落后两个阶段。
前面提到，对每个线程来说，不会再访问其当前本地阶段的前一个阶段的待回收对象，
如果所有线程的本地阶段都一致，处于同一个阶段，那么前一个阶段的待回收对象就是可以被安全回收的，
在加上线程的本地阶段最多落后全局阶段一个阶段，而且每个线程调用`EB::leave()`时都用其本地阶段标识待回收对象，
...
EB的线程本地状态也可以实现无锁化：
* 线程的本地队列的访问和修改无需加锁，因为线程内部的操作都是串行的；
* 线程本地阶段采用原子变量；
* 标记当前线程的状态也无需加锁。

不同线程并发调用`EB::enter()`导致的竞争主要集中在对全局阶段和全局队列的修改：
* 不同线程竞争修改全局阶段至下一个阶段并不会导致不一致状态，一方面由于全局阶段是原子变量，另一方面不同线程对全局阶段的修改都是改为下一个阶段，这是阶段跨越条件决定的；
