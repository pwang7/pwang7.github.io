...
然后，EB为每个线程维护一个本地队列，用于保存该线程待回收的对象，同时EB为每个线程用线程本地变量维护一份EB全局阶段的副本（称为本地阶段）。
接下来每个线程把要访问共享数据进行修改的操作，都用`EB::enter()`和`EB::leave()`保护起来，并且把待回收的对象指针通过`EB::leave()`传给EB来后续回收。
`EB::enter()`和`EB::leave()`的实现如下所示。

`EB::enter()`的操作：
1. 如果某线程是第一次调用`EB::enter()`，则为该线程创建一个本地队列，用于保存该线程待回收的对象，并为其创建本地阶段的变量；
2. 读取当前EB全局阶段，并复制给该线程的本地阶段变量；
3. 尝试跨越至下一全局阶段并尝试回收内存：
    - 检查每个线程的本地阶段是否一致，即是否都处于同一阶段，如有不一致则放弃，转去执行第4步；
    - 如果上面条件满足，则EB的全局阶段跨越至下个阶段，并从EB维护的全局队列中回收当前全局阶段的两个阶段前的对象的内存，然后执行第4步；
4. 标记当前线程正在修改共享数据；

可以看出，`EB::enter()`负责标识某个线程正在修改共享数据，并尝试把全局阶段跨越至下个阶段以回收内存。
...
当然`EB::enter()`并不能保证每次被调用就一定能成功回收内存，因为要先满足阶段跨越的条件，满足之后才能回收内存。
...
可以看出，`EB::leave()`比较简单。有一个细节要注意，`EB::leave()`给输入的待回收对象指针标记其所处线程的本地阶段，而不是用全局阶段标记，其原因下文解释。

### 为什么EB能安全回收内存？

那为什么EB回收当前全局阶段前两个阶段的对象是安全的呢？

首先，在一个线程里，待回收对象只会在某个本地阶段交给EB来回收，即每个待回收对象只关联一个阶段。
线程里对每个待回收对象只会传给`EB::leave()`一次（`EB::leave()`会为待回收对象标记当前本地阶段），不然会出现double delete。
一个待回收对象在当前阶段交给EB之后，线程只能在当前阶段内可以访问该对象。
如果线程在下个阶段还要访问该对象，那这个对象就不应该是当前阶段待回收的，而`EB::leave()`只应该用于收集线程在当前阶段待回收的对象。
因此，当一个线程处于某个阶段，这个线程不会再访问前一个阶段的待回收对象，即保留着前一个阶段的待回收对象的指针。
怎么理解呢？举个例子来说明。下面是一段对无锁化堆栈执行入栈和出栈操作的线程函数代码片段：
```
while true {
    Node * pop_node = pop();
    if ( pop_node ) {
...
```
上面的线程函数代码里用循环不停地调用无锁化堆栈的出栈操作并消费出栈节点。由上面的例子可以看出：
- 线程在循环体内调用`EB::enter()`和`EB::leave()`实现本地阶段不停地跨越至下个阶段，即线程函数循环调用出栈操作，出栈操作里的`EB::enter()`有可能改变线程的本地阶段；
- 待回收对象——栈顶节点在出栈操作返回前被传入`EB::leave()`以备回收，出栈操作后对栈顶节点的访问在下次调用`EB::enter()`前结束，即对栈顶节点的访问在下次出栈操作前完成，而下次出栈有可能使线程进入下一个阶段；
...
回顾`EB::enter()`里第三步检查阶段跨越的条件：全局阶段跨越至下个阶段前，所有线程的本地阶段都一致。
...
此外，线程的当前本地阶段最多落后全局阶段一个阶段，而且每个线程调用`EB::leave()`时都用其当前本地阶段标识待回收对象，
...
类似的，不同线程并发调用`EB::leave()`导致的竞争主要集中在对全局队列的修改：
* 不同线程竞争修改全局队列，把本地队列的元素加入全局队列，由于全局队列是无锁化队列，因此也不会导致不一致状态。

总结一下，EB是无锁化编程场景的一种确定型GC实现，其回收策略简单，延迟批量回收，因而对性能影响小。
[libcds](https://github.com/khizmax/libcds)里已经有EB的C++实现。
