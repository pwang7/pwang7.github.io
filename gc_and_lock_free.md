...
```
入栈的每个节点都是new出来的，所以可能觉得想当然出栈之后的每个节点在消费过过以后要被delete掉。
...
从上面无锁化堆栈的例子可以看出，Java的GC一定程度简化了无锁化编程，因为不用考虑内存回收的问题，Java的GC会安全的回收内存，
...
对没有GC的语言，比如C++，已经有一些针对无锁化编程的内存回收算法的实现，比如`libcds`，
更进一步，`libcds`还实现了无锁化堆栈、无锁化队列、无锁化哈希表等等。
